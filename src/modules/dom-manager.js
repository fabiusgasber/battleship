...
const domManager = (() => {
  ...
  const handleDragOver = (e) => {
    ...
    if(gameboard && gameboard.getAttribute("id") === "user-gameboard") e.preventDefault();
  }
  ...
  const handleDrop = (e) => {
    ...
    if(uiBoard.getAttribute("id") === "user-gameboard" && dragged && draggedShip){
      e.preventDefault();
      const children = Array.from(dragged.children);
      const length = parseInt(dragged.getAttribute("length"));
      if(document.querySelector(".ship-container").classList.contains("rotate")) {
        gameInfo.textContent = "User place your ships..."
        rowIndex = parseInt(rowIndex) - children.indexOf(draggedShip);
        colIndex = parseInt(colIndex); 
        user.getGameboard().placeShip(new Ship(length), [rowIndex, colIndex], "v");
      }
      else if(!document.querySelector(".ship-container").classList.contains("rotate")){
        gameInfo.textContent = "User place your ships..."
        rowIndex = parseInt(rowIndex);
        colIndex = parseInt(colIndex) - children.indexOf(draggedShip);
        user.getGameboard().placeShip(new Ship(length), [rowIndex, colIndex], "h");  
      }
    }
  ...
  const takeTurns = (userTurn) => {
    ...
    if(user.getGameboard().ships.length === SHIPS.length && computer.getGameboard().ships.length === SHIPS.length){
      if (userTurn && !isEnd()){
